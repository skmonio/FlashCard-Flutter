...
class FriendsService {
  ...
  Future<List<Map<String, dynamic>>> getFriendDecks(String friendId) async {
    try {
      ...
      try {
        ...
        print('🔍 FriendsService: Found ${decks.length} public decks for friend $friendId');
        ...
        print('⚠️ is_public column not found in decks table, showing all decks: $e');
        final response = await _client
            .from('decks')
            .select('*')
            .eq('user_id', friendId)
        ...
        final decks = List<Map<String, dynamic>>.from(response);
        print('🔍 FriendsService: Found ${decks.length} total decks for friend $friendId (fallback)');
        return decks;
      }
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> getFriendCards(String friendId) async {
    try {
      ...
      try {
        ...
        final publicCards = List<Map<String, dynamic>>.from(publicCardsResponse);
        print('🔍 FriendsService: Found ${publicCards.length} individually public cards');
      ...
      try {
        ...
        final publicDeckIds = publicDecksResponse.map((deck) => deck['id'] as String).toList();
        ...
        if (publicDeckIds.isNotEmpty) {
          ...
          print('🔍 FriendsService: Found ${publicDeckCardsResponse.length} cards from public decks via deck_cards');
          ...
          if (publicDeckCardsResponse.isEmpty) {
            print('🔍 FriendsService: No deck_cards relationships found for public decks');
            print('🔍 FriendsService: This means when deck is public, all cards should be public');
            print('🔍 FriendsService: Using fallback - all friend cards are considered public');
            ...
            print('🔍 FriendsService: Found ${allFriendCardsResponse.length} total friend cards (fallback for public decks)');
      ...
      if (allCards.isEmpty) {
        print('⚠️ No public cards found - no public decks and no individual public cards');
        print('🔍 FriendsService: This friend has no public content to share');
      }
      
      print('🔍 FriendsService: Total cards available to friend: ${allCards.length}');
  ...
  Future<List<Map<String, dynamic>>> getFriendDeckCards(String friendId, String deckId) async {
    try {
      print('🔍 FriendsService: Getting cards for friend $friendId in deck $deckId');
      ...
      print('🔍 FriendsService: Deck info: ${deckResponse['name']} (public: ${deckResponse['is_public']})');
      ...
      print('🔍 FriendsService: Total cards in deck (any user): ${allDeckCardsResponse.length}');
      if (allDeckCardsResponse.isNotEmpty) {
        print('🔍 FriendsService: First card user_id: ${allDeckCardsResponse.first['flashcards']['user_id']}');
      ...
      print('🔍 FriendsService: Friend has ${friendCardsResponse.length} total cards');
      if (friendCardsResponse.isNotEmpty) {
        ...
        for (int i = 0; i < friendCardsResponse.length && i < 3; i++) {
          ...
          print('  - Card: ${card['word']} -> ${card['definition']} (id: ${card['id']})');
        }
      }
      ...
      print('🔍 FriendsService: Friend has ${friendCardsInAnyDeck.length} cards linked to decks');
      ...
      final cards = deckCardsResponse.map((deckCard) => deckCard['flashcards'] as Map<String, dynamic>).toList();
      
      print('🔍 FriendsService: Found ${cards.length} cards in deck $deckId for friend $friendId');
      for (final card in cards) {
        print('  - Card: ${card['word']} -> ${card['definition']}');
      }
      
      // If no cards found and friend has cards but no deck relationships, simulate relationships locally
      if (cards.isEmpty && friendCardsResponse.isNotEmpty && friendCardsInAnyDeck.isEmpty) {
        print('🔍 FriendsService: No deck_cards relationships found.');
        print('🔍 FriendsService: Friend has ${friendCardsResponse.length} cards but 0 deck relationships.');
        print('🔍 FriendsService: This suggests the friend\'s data was imported before deck_cards table was populated.');
        print('🔍 FriendsService: Simulating deck-card relationships locally...');
        
        // Simulate deck-card relationships locally without inserting into database
        final simulatedCards = await _simulateDeckCardRelationships(friendId, deckId, friendCardsResponse);
        print('🔍 FriendsService: Simulated ${simulatedCards.length} cards for deck $deckId');
  ...
  Future<bool> populateDeckCardsForSpecificDeck(String friendId, String deckId, List<Map<String, dynamic>> friendCards) async {
    try {
      print('🔍 FriendsService: Populating deck_cards for specific deck: $deckId');
      ...
      if (existingRelationships.isNotEmpty) {
        print('🔍 FriendsService: Deck_cards relationships already exist for deck $deckId (${existingRelationships.length} found)');
        return true;
      }
      ...
      for (final card in friendCards) {
        try {
          ...
          if (successCount % 50 == 0) {
            print('🔍 FriendsService: Created $successCount relationships for deck $deckId...');
          }
        } catch (e) {
          // Check if it's a duplicate key error - if so, it's actually fine
          if (e.toString().contains('duplicate key value violates unique constraint')) {
            print('🔍 FriendsService: Relationship already exists, skipping: $deckId -> ${card['id']}');
            successCount++; // Count as success since the relationship exists
          } else {
            errorCount++;
            print('❌ Error creating relationship for card ${card['id']} in deck $deckId: $e');
            
            // If we get too many errors, stop
            if (errorCount > 10) {
              print('❌ Too many errors, stopping deck_cards population for deck $deckId');
              break;
            }
          }
        }
      ...
      if (successCount > 0) {
        print('✅ FriendsService: Successfully created $successCount deck_cards relationships for deck $deckId');
        if (errorCount > 0) {
          print('⚠️ FriendsService: $errorCount relationships failed to create for deck $deckId');
        }
        return true;
      } else {
        print('❌ FriendsService: Failed to create any deck_cards relationships for deck $deckId');
        return false;
      }
    } catch (e) {
  ...
  Future<bool> populateFriendDeckCards(String friendId) async {
    try {
      print('🔍 FriendsService: Starting deck_cards population for friend: $friendId');
      ...
      print('🔍 FriendsService: Found ${friendCardsResponse.length} cards and ${friendDecksResponse.length} decks');
      ...
      if (existingRelationships.isNotEmpty) {
        print('🔍 FriendsService: Deck_cards relationships already exist for friend $friendId');
        return true;
      }
      ...
      print('🔍 FriendsService: Creating deck_cards relationships for ${friendCardsResponse.length} cards across ${friendDecksResponse.length} decks');
        ...
        for (final card in friendCardsResponse) {
          try {
            ...
            if (successCount % 100 == 0) {
              print('🔍 FriendsService: Created $successCount relationships so far...');
            }
          } catch (e) {
            errorCount++;
            print('❌ Error creating relationship for card ${card['id']} in deck ${deck['id']}: $e');
      ...
      if (successCount > 0) {
        print('✅ FriendsService: Successfully created $successCount deck_cards relationships for friend $friendId');
        if (errorCount > 0) {
          print('⚠️ FriendsService: $errorCount relationships failed to create');
        }
        return true;
      } else {
        print('❌ FriendsService: Failed to create any deck_cards relationships');
        return false;
      }
  ...
  Future<int> getFriendDeckCardCount(String friendId, String deckId) async {
    try {
      print('🔍 FriendsService: Getting card count for friend $friendId in deck $deckId');
      ...
      if (deckCardsResponse.isNotEmpty) {
        print('🔍 FriendsService: Found ${deckCardsResponse.length} cards in deck via deck_cards table');
      ...
      if (friendCardsResponse.isNotEmpty) {
        print('🔍 FriendsService: Friend has ${friendCardsResponse.length} total cards but no deck_cards relationships');
        print('🔍 FriendsService: This suggests the friend\'s data was imported before deck_cards table was populated');
        print('🔍 FriendsService: Simulating deck-card relationships for count...');
  ...
  /// Simulate deck-card relationships locally without inserting into database
  Future<List<Map<String, dynamic>>> _simulateDeckCardRelationships(String friendId, String deckId, List<Map<String, dynamic>> allFriendCards) async {
    try {
      print('🔍 FriendsService: Simulating deck-card relationships for deck $deckId');
      ...
      final currentDeck = friendDecksResponse.where(
        (deck) => deck['id'] == deckId,
      ).firstOrNull;
      ...
      if (isDefaultDeck) {
        ...
        print('🔍 FriendsService: Default deck "$deckName" - simulating ${simulatedCards.length} cards');
      } else {
        // For custom decks, distribute cards more evenly
        final customDecks = friendDecksResponse.where((deck) => !defaultDeckNames.contains(deck['name'])).toList();
        final deckIndex = customDecks.indexWhere((deck) => deck['id'] == deckId);
        
        if (deckIndex != -1) {
          // Calculate which cards should belong to this deck
          final cardsForDefaultDecks = 15; // 10 for Uncategorized + 5 for Review
          final remainingCards = allFriendCards.skip(cardsForDefaultDecks).toList();
          
          if (remainingCards.isNotEmpty && customDecks.isNotEmpty) {
            final cardsPerDeck = (remainingCards.length / customDecks.length).ceil();
            final startIndex = deckIndex * cardsPerDeck;
            final endIndex = (startIndex + cardsPerDeck).clamp(0, remainingCards.length);
        ...
        print('🔍 FriendsService: Custom deck "$deckName" - simulating ${simulatedCards.length} cards');
      }
  ...
  Future<bool> _populateFriendDeckCardsInferred(String friendId) async {
    try {
      print('🔍 FriendsService: Starting inferred deck_cards population for friend: $friendId');
      ...
      if (friendDecksResponse.isEmpty || friendCardsResponse.isEmpty) {
        print('🔍 FriendsService: No decks or cards found for friend $friendId');
        return false;
      }
      
      print('🔍 FriendsService: Found ${friendDecksResponse.length} decks and ${friendCardsResponse.length} cards');
      
      // Strategy: Distribute cards evenly across decks, with some logic for default decks
      final defaultDeckNames = {'Uncategorized', 'Review'};
      final customDecks = friendDecksResponse.where((deck) => !defaultDeckNames.contains(deck['name'])).toList();
      final defaultDecks = friendDecksResponse.where((deck) => defaultDeckNames.contains(deck['name'])).toList();
      ...
      for (final deck in defaultDecks) {
        ...
        final cardsForThisDeck = friendCardsResponse.take(deckName == 'Uncategorized' ? 10 : 5).toList();
      ...
      final remainingCards = friendCardsResponse.skip(15).toList(); // Skip cards already assigned to default decks
      ...
      if (successCount > 0) {
        print('✅ FriendsService: Successfully created $successCount deck_cards relationships');
        if (errorCount > 0) {
          print('⚠️ FriendsService: $errorCount relationships failed to create');
        }
        return true;
  ...
  Future<Map<String, dynamic>> getFriendStats(String friendId) async {
    try {
      ...
      final profileResponse = await _client
          .from('user_profiles')
          .select('level, xp, current_streak, total_sessions, total_cards_studied')
          .eq('id', friendId)
          .single();
      ...
      try {
        try {
          ...
          print('⚠️ is_public column not found in decks table, counting all decks');
          final deckCountResponse = await _client
              .from('decks')
              .select('id')
              .eq('user_id', friendId);
      ...
      try {
        try {
          ...
          print('⚠️ is_public column not found in flashcards table, counting all cards');
          final cardCountResponse = await _client
              .from('flashcards')
              .select('id')
              .eq('user_id', friendId);
      ...
      return {
        ...
        'total_study_time': profileResponse['total_sessions'] ?? 0, // Use total_sessions as proxy
        'cards_created': profileResponse['total_cards_studied'] ?? 0, // Use total_cards_studied as proxy
  ...
  /// Copy friend's deck
  Future<void> copyFriendDeck(String friendId, String deckId, String newDeckName, {Function(int current, int total)? onProgress}) async {
    try {
      final userId = SupabaseService.instance.currentUser!.id;
      ...
      final newDeckResponse = await _client
          ...
          .insert({
            'name': newDeckName,
            'description': '${deckResponse['description'] ?? ''} (Copied from ${deckResponse['name']})',
            'user_id': userId,
      ...
      print('🔍 FriendsService: Looking for cards in deck $deckId for friend $friendId');
      ...
      print('🔍 FriendsService: Found ${deckCardsResponse.length} cards to copy');
      ...
      if (deckCardsResponse.isEmpty && allDeckCardsResponse.isEmpty) {
        print('🔍 FriendsService: No deck_cards relationships found.');
        print('🔍 FriendsService: Simulating deck-card relationships for copying...');
        ...
        if (friendCardsResponse.isNotEmpty) {
          ...
          cardsToCopy = await _simulateDeckCardRelationships(friendId, deckId, friendCardsResponse);
          print('🔍 FriendsService: Simulated ${cardsToCopy.length} cards to copy for deck $deckId');
        } else {
        ...
        cardsToCopy = deckCardsResponse.map((deckCard) => deckCard['flashcards'] as Map<String, dynamic>).toList();
      }
      ...
      for (final card in cardsToCopy) {
        copiedCount++;
        print('🔍 FriendsService: Copying card $copiedCount/$totalCards: "${card['word']}" -> "${card['definition']}"');
        ...
        try {
          ...
          try {
            ...
            print('🔍 FriendsService: Added card $newCardId to deck $newDeckId');
          } catch (e) {
            // Check if it's a duplicate key error - if so, it's actually fine
            if (e.toString().contains('duplicate key value violates unique constraint')) {
              print('🔍 FriendsService: Card $newCardId already in deck $newDeckId, skipping');
            } else {
              print('❌ FriendsService: Error adding card $newCardId to deck $newDeckId: $e');
  ...
  /// Copy friend's card
  Future<void> copyFriendCard(String friendId, String cardId, List<String> targetDeckIds) async {
    try {
      final userId = SupabaseService.instance.currentUser!.id;
      ...
      print('🔍 FriendsService: Copying card: "${cardResponse['word']}" -> "${cardResponse['definition']}"');
      ...
      for (final deckId in targetDeckIds) {
        try {
          ...
          print('🔍 FriendsService: Successfully added card $newCardId to deck $deckId');
        } catch (e) {
          // Check if it's a duplicate key error - if so, it's actually fine
          if (e.toString().contains('duplicate key value violates unique constraint')) {
            print('🔍 FriendsService: Card $newCardId already in deck $deckId, skipping');
          } else {
            print('❌ FriendsService: Error adding card $newCardId to deck $deckId: $e');
          }
        }
      }
