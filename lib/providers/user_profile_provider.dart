...
class UserProfileProvider extends ChangeNotifier {
  ...
  Future<void> _loadFromStorage() async {
    ...
    if (profileJson != null) {
      try {
        ...
        print(
          '🔍 UserProfileProvider: Loaded profile - XP: ${_profile.xp}, Level: ${_profile.level}',
        );
      } catch (e) {
        print(
          '🔍 UserProfileProvider: Error loading profile, using default: $e',
        );
      ...
      print(
        '🔍 UserProfileProvider: No profile found in storage, using default',
      );
    }
  }
  ...
  Future<void> _saveToStorage() async {
    try {
      ...
      print(
        '🔍 UserProfileProvider: Profile saved to SharedPreferences successfully',
      );
      ...
      if (SupabaseService.instance.isAuthenticated) {
        ...
        try {
          final syncResult = await DataSyncService.syncUserProfile();
          if (syncResult.isFailure) {
            _error = syncResult.message;
            notifyListeners();
            print(
              '🔍 UserProfileProvider: Profile auto-sync failed: ${syncResult.message}',
            );
          } else if (syncResult.isSuccess) {
            print(
              '🔍 UserProfileProvider: Profile auto-sync completed successfully',
            );
          }
        } catch (e) {
          print(
            '🔍 UserProfileProvider: Profile auto-sync failed (non-critical): $e',
          );
  ...
  Future<void> addXp(int xpToAdd) async {
    ...
    print(
      '🔍 UserProfileProvider: Current profile XP: ${_profile.xp}, Level: ${_profile.level}',
    );
    ...
    print(
      '🔍 UserProfileProvider: Old XP: $oldXp, New XP: $newXp, Old Level: $oldLevel',
    );
    ...
    print(
      '🔍 UserProfileProvider: XP for current level ${newLevel}: ${_getXpForLevel(newLevel)}',
    );
    print(
      '🔍 UserProfileProvider: XP for next level ${newLevel + 1}: ${_getXpForLevel(newLevel + 1)}',
    );

    _profile = _profile.copyWith(xp: newXp, level: newLevel);

    print(
      '🔍 UserProfileProvider: After copyWith - XP: ${_profile.xp}, Level: ${_profile.level}',
    );
    print(
      '🔍 UserProfileProvider: Progress to next level: ${_profile.progressToNextLevel}',
    );
    ...
    notifyListeners();
    print(
      '🔍 UserProfileProvider: notifyListeners called - XP update complete',
    );
  }
  ...
  Future<void> updateSessionStats({
    ...
    final totalAccuracy =
        (_profile.accuracy * _profile.totalSessions) + sessionAccuracy;
    final newAccuracy = (totalAccuracy / newTotalSessions).clamp(
      0.0,
      1.0,
    ); // Clamp to 0-100%
  ...
  Future<void> updateStreakFromStudyActivity() async {
    ...
    if (_profile.lastStudyDate == null) {
      ...
      if (daysDifference == 0) {
        ...
        final newBestStreak = newStreak > _profile.bestStreak
            ? newStreak
            : _profile.bestStreak;
      ...
      if (!achievement.isUnlocked) {
        ...
        switch (achievement.type) {
          ...
          case AchievementType.accuracy:
            shouldUnlock =
                _profile.accuracy >= (achievement.xpRequired / 100.0);
            break;
        }
